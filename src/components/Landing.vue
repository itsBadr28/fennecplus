<script setup lang="ts">
</script>

<template>
  <div class=" wscreen hscreen flex" >
        <div class="flex wfull ma">
            <div class="flex w100% ma">
                <div class="w55% flex">
                    <img src="../assets/hero.png" class="w95% mta  mla" alt="">
                </div>
                <div class=" w45% flex" >
                    <div class="mta mla" dir="rtl">
                        <h2  class=" font-vip my4 text-4rem line-height-4.5rem text-white">عـــش الـتــجـربـة <br> و إستمتع بالمغامرة</h2>
                        <h3 class="m0 mb4 text-2xl  text-white font-cairo">
                            اكتشف تجربة IPTV الجزائرية المثالية مع  Fennec Plus<br> ابتداء من 500 دج/شهر فقط 
                        </h3>
                        <button class="bg-#96c834 w3/5 my3">
                            <h2 class="text-3xl text-black uppercase my2 font-lemon ">
                                get now
                            </h2>
                        </button>
                    </div>
                </div>  
            </div>
             
        </div>
    </div>
</template>

<style scoped>
.read-the-docs {
  color: #888;
}
</style>
